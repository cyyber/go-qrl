...
func TestFieldSub(t *testing.T) {
	...
	ns.SubscribeField(fields[0], func(n *qnode.Node, state Flags, oldValue, newValue any) {
	...
	ns2 := NewNodeStateMachine(mdb, []byte("-ns"), clock, s)
	ns2.SubscribeField(fields[0], func(n *qnode.Node, state Flags, oldValue, newValue any) {
		lastState, lastOldValue, lastNewValue = state, oldValue, newValue
	})
	ns2.Start()
