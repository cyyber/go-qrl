...
type (
	...
	FieldCallback func(n *qnode.Node, state Flags, oldValue, newValue any)
...
func (ns *NodeStateMachine) GetField(n *qnode.Node, field Field) any {
	ns.lock.Lock()
	defer ns.lock.Unlock()
...
func (ns *NodeStateMachine) SetField(n *qnode.Node, field Field, value any) error {
	ns.lock.Lock()
	defer ns.lock.Unlock()
...
// from a subscription/operation callback).
func (ns *NodeStateMachine) SetFieldSub(n *qnode.Node, field Field, value any) error {
	ns.lock.Lock()
	defer ns.lock.Unlock()
