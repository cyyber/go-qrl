...
func NewKeyStoreTransactorWithChainID(keystore *keystore.KeyStore, account accounts.Account, chainID *big.Int) (*TransactOpts, error) {
	...
	return &TransactOpts{
		...
		Signer: func(address common.Address, tx *types.Transaction) (*types.Transaction, error) {
			...
			extraParams := []byte{}
			sig, err := keystore.SignHash(account, signer.Hash(tx, desc, extraParams).Bytes())
			if err != nil {
				return nil, err
			}
			...
			return tx.WithAuthValues(signer, sig, pk, desc, extraParams)
		},
...
func NewKeyedTransactorWithChainID(w wallet.Wallet, chainID *big.Int) (*TransactOpts, error) {
	...
	return &TransactOpts{
		...
		Signer: func(address common.Address, tx *types.Transaction) (*types.Transaction, error) {
			...
			extraParams := []byte{}
			sig, err := pqcrypto.Sign(signer.Hash(tx, desc, extraParams).Bytes(), w)
			if err != nil {
				return nil, err
			}
			pk := w.GetPK()
