...
type binder struct {
	...
	// and the hyperion type signature of the struct
...
func BindV2(types []string, abis []string, bytecodes []string, pkg string, libs map[string]string, aliases map[string]string) (string, error) {
	...
	for i := 0; i < len(types); i++ {
		...
		err = iterSorted(qrvmABI.Methods, func(_ string, original abi.Method) error {
			return cb.bindMethod(original)
		})
		if err != nil {
			return "", err
		}
		err = iterSorted(qrvmABI.Events, func(_ string, original abi.Event) error {
			return cb.bindEvent(original)
		})
		if err != nil {
			return "", err
		}
		err = iterSorted(qrvmABI.Errors, func(_ string, original abi.Error) error {
			return cb.bindError(original)
		})
		if err != nil {
			return "", err
		}
		b.contracts[types[i]] = newTmplContractV2(types[i], abis[i], bytecodes[i], qrvmABI.Constructor, cb)
	}
